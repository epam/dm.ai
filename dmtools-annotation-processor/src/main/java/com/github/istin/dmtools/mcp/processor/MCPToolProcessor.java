...
public class MCPToolProcessor extends AbstractProcessor {
    ...
    private void generateParameterConversionMethod(PrintWriter out) {
        ...
        out.println("        }");
        out.println();
        out.println("        // Handle Number to String conversion (JS bridge passes integral numbers");
        out.println("        // such as parseInt results as Integer/Long/Double for string-typed params)");
        out.println("        if (targetType.equals(\"java.lang.String\") && value instanceof Number) {");
        out.println("            Number num = (Number) value;");
        out.println("            double d = num.doubleValue();");
        out.println("            if (d == Math.floor(d) && !Double.isInfinite(d)) {");
        out.println("                return (T) String.valueOf(num.longValue());");
        out.println("            }");
        out.println("            return (T) num.toString();");
        out.println("        }");
        out.println();
        out.println("        // Handle Number to Integer/Long/Double conversion (JS bridge may box an");
        out.println("        // integral JS number as Long or Double, not necessarily the declared type)");
        out.println("        if ((targetType.equals(\"java.lang.Integer\") || targetType.equals(\"int\")) && value instanceof Number) {");
        out.println("            return (T) Integer.valueOf(((Number) value).intValue());");
        out.println("        }");
        out.println("        if ((targetType.equals(\"java.lang.Long\") || targetType.equals(\"long\")) && value instanceof Number) {");
        out.println("            return (T) Long.valueOf(((Number) value).longValue());");
        out.println("        }");
        out.println("        if ((targetType.equals(\"java.lang.Double\") || targetType.equals(\"double\")) && value instanceof Number) {");
        out.println("            return (T) Double.valueOf(((Number) value).doubleValue());");
        out.println("        }");
        out.println();
        out.println("        // Handle String to Boolean conversion");
        out.println("        if ((targetType.equals(\"java.lang.Boolean\") || targetType.equals(\"boolean\")) && value instanceof String) {");
        out.println("            return (T) Boolean.valueOf((String) value);");
        out.println("        }");
        out.println();
        out.println("        // Handle String to Integer conversion");
        out.println("        if ((targetType.equals(\"java.lang.Integer\") || targetType.equals(\"int\")) && value instanceof String) {");
        out.println("            try {");
        out.println("                return (T) Integer.valueOf((String) value);");
        out.println("            } catch (NumberFormatException e) {");
        out.println("                throw new IllegalArgumentException(\"Parameter '\" + paramName + \"' must be a valid integer. Received: '\" + value + \"'\", e);");
        out.println("            }");
        out.println("        }");
        out.println();
        out.println("        // Handle String to Long conversion");
        out.println("        if ((targetType.equals(\"java.lang.Long\") || targetType.equals(\"long\")) && value instanceof String) {");
        out.println("            try {");
        out.println("                return (T) Long.valueOf((String) value);");
        out.println("            } catch (NumberFormatException e) {");
        out.println("                throw new IllegalArgumentException(\"Parameter '\" + paramName + \"' must be a valid long. Received: '\" + value + \"'\", e);");
        out.println("            }");
        out.println("        }");
        out.println();
        out.println("        // Handle String to Double conversion");
        out.println("        if ((targetType.equals(\"java.lang.Double\") || targetType.equals(\"double\")) && value instanceof String) {");
        out.println("            try {");
        out.println("                return (T) Double.valueOf((String) value);");
        out.println("            } catch (NumberFormatException e) {");
        out.println("                throw new IllegalArgumentException(\"Parameter '\" + paramName + \"' must be a valid number. Received: '\" + value + \"'\", e);");
        out.println("            }");
        out.println("        }");
        out.println();
        ...
        out.println("        }");
        out.println();
        out.println("        // Handle String to File conversion");
        out.println("        if (targetType.equals(\"java.io.File\") && value instanceof String) {");
        out.println("            return (T) new java.io.File((String) value);");
        out.println("        }");
        out.println();
        ...
        out.println("        }");
        out.println();
        out.println("        // Handle JSONObject → custom model class (e.g. Attachment, Content) via reflection");
        out.println("        if (value instanceof org.json.JSONObject) {");
        out.println("            try {");
        out.println("                Class<?> targetClass = Class.forName(targetType);");
        out.println("                try {");
        out.println("                    java.lang.reflect.Constructor<?> ctor = targetClass.getConstructor(org.json.JSONObject.class);");
        out.println("                    return (T) ctor.newInstance((org.json.JSONObject) value);");
        out.println("                } catch (NoSuchMethodException e2) {");
        out.println("                    java.lang.reflect.Constructor<?> ctor = targetClass.getConstructor(String.class);");
        out.println("                    return (T) ctor.newInstance(((org.json.JSONObject) value).toString());");
        out.println("                }");
        out.println("            } catch (Exception e) {");
        out.println("                // fall through to direct cast");
        out.println("            }");
        out.println("        }");
        out.println();
        out.println("        // Handle String → custom model class via reflection");
        out.println("        if (value instanceof String) {");
        out.println("            String strValue = (String) value;");
        out.println("            if (strValue.trim().startsWith(\"{\") || strValue.trim().startsWith(\"[\")) {");
        out.println("                try {");
        out.println("                    Class<?> targetClass = Class.forName(targetType);");
        out.println("                    try {");
        out.println("                        java.lang.reflect.Constructor<?> ctor = targetClass.getConstructor(String.class);");
        out.println("                        return (T) ctor.newInstance(strValue);");
        out.println("                    } catch (NoSuchMethodException e2) {");
        out.println("                        java.lang.reflect.Constructor<?> ctor = targetClass.getConstructor(org.json.JSONObject.class);");
        out.println("                        return (T) ctor.newInstance(new org.json.JSONObject(strValue));");
        out.println("                    }");
        out.println("                } catch (Exception e) {");
        out.println("                    // fall through to direct cast");
        out.println("                }");
