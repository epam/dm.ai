...
|-----------|-------------|------------|
| `jira_add_fix_version` | Add a fix version to a Jira ticket (without removing existing ones) | `key` (string, **required**)<br>`fixVersion` (string, **required**) |
...
| `jira_assign_ticket_to` | Assigns a Jira ticket to user | `key` (string, **required**)<br>`accountId` (string, **required**) |
| `jira_assign_workflow_scheme` | Assign a workflow scheme (by its numeric ID) to a classic Jira project (by its numeric project ID). Requires Jira admin. | `workflowSchemeId` (string, **required**)<br>`projectId` (string, **required**) |
| `jira_attach_file_to_ticket` | Attach a file to a Jira ticket from a local file path. The file will only be attached if a file with the same name doesn't already exist | `ticketKey` (string, **required**)<br>`name` (string, **required**)<br>`contentType` (string, optional)<br>`filePath` (string, **required**) |
| `jira_clear_field` | Clear (delete value) a specific field value in a Jira ticket | `key` (string, **required**)<br>`field` (string, **required**) |
...
| `jira_create_project_issue_type` | Create a project-scoped issue type in a next-gen Jira project. Use type 'subtask' for subtasks and 'standard' for all others. Skips if the name already exists. Requires Jira admin. | `projectKey` (string, **required**)<br>`name` (string, **required**)<br>`type` (string, optional)<br>`description` (string, optional) |
| `jira_create_ticket_basic` | Create a new Jira ticket with basic fields (project, issue type, summary, description) | `project` (string, **required**)<br>`issueType` (string, **required**)<br>`summary` (string, **required**)<br>`description` (string, **required**) |
...
| `jira_create_ticket_with_parent` | Create a new Jira ticket with a parent relationship | `project` (string, **required**)<br>`issueType` (string, **required**)<br>`summary` (string, **required**)<br>`description` (string, **required**)<br>`parentKey` (string, **required**) |
...
| `jira_get_comments` | Get all comments for a specific Jira ticket | `key` (string, **required**)<br>`ticket` (object, optional) |
...
| `jira_get_ticket` | Get a specific Jira ticket by key with optional field filtering | `key` (string, **required**)<br>`fields` (array, optional) |
...
| `jira_link_issues` | Link two Jira issues with a specific relationship type | `sourceKey` (string, **required**)<br>`anotherKey` (string, **required**)<br>`relationship` (string, **required**) |
| `jira_move_to_status` | Move a Jira ticket to a specific status (workflow, transition) | `key` (string, **required**)<br>`statusName` (string, **required**) |
| `jira_move_to_status_with_resolution` | Move a Jira ticket to a specific status (workflow, transition) with resolution | `key` (string, **required**)<br>`statusName` (string, **required**)<br>`resolution` (string, **required**) |
...
| `jira_remove_fix_version` | Remove a fix version from a Jira ticket | `key` (string, **required**)<br>`fixVersion` (string, **required**) |
...
| `jira_search_by_page` | Search for Jira tickets using JQL with paging support | `jql` (string, **required**)<br>`nextPageToken` (string, **required**)<br>`fields` (array, **required**) |
| `jira_search_with_pagination` | [Deprecated] Search for Jira tickets using JQL with pagination support | `jql` (string, **required**)<br>`startAt` (number, **required**)<br>`fields` (array, **required**) |
| `jira_set_fix_version` | Set the fix version for a Jira ticket | `key` (string, **required**)<br>`fixVersion` (string, **required**) |
| `jira_set_priority` | Set the priority for a Jira ticket | `key` (string, **required**)<br>`priority` (string, **required**) |
...
| `jira_update_all_fields_with_name` | Update ALL fields with the same name in a Jira ticket. Useful when there are multiple custom fields with the same display name. | `key` (string, **required**)<br>`fieldName` (string, **required**)<br>`value` (object, **required**) |
| `jira_update_description` | Update the description of a Jira ticket. Supports Jira markup syntax: h2. for headings, *text* for bold, {code}text{code} for inline code, * for bullet lists | `key` (string, **required**)<br>`description` (string, **required**) |
| `jira_update_field` | Update field(s) in a Jira ticket. When using field names (e.g., 'Dependencies'), updates ALL fields with that name. When using custom field IDs (e.g., 'customfield_10091'), updates only that specific field. | `key` (string, **required**)<br>`field` (string, **required**)<br>`value` (object, **required**) |
| `jira_update_field_as_adf` | Update a rich-text field in a Jira ticket using Jira API v3 and Atlassian Document Format (ADF). The value can be a plain string (wrapped into a paragraph) or a JSON object/array representing an ADF document. Required for interactive task-list checkboxes in Jira Cloud. | `key` (string, **required**)<br>`field` (string, **required**)<br>`value` (object, **required**) |
| `jira_update_ticket` | Update a Jira ticket using JSON parameters following the standard Jira REST API format | `key` (string, **required**)<br>`params` (object, **required**) |
...
| `jira_xray_add_preconditions_to_test` | Add multiple preconditions to a test issue using X-ray GraphQL API. Returns JSONArray of results. | `testIssueId` (string, **required**)<br>`preconditionIssueIds` (object, **required**) |
| `jira_xray_add_test_step` | Add a single test step to a test issue using X-ray GraphQL API. Returns JSONObject with created step details. | `issueId` (string, **required**)<br>`action` (string, **required**)<br>`data` (string, optional)<br>`result` (string, optional) |
| `jira_xray_add_test_steps` | Add multiple test steps to a test issue using X-ray GraphQL API. Returns JSONArray of created step objects. | `issueId` (string, **required**)<br>`steps` (object, **required**) |
| `jira_xray_create_precondition` | Create a Precondition issue in Xray with optional steps (converted to definition). Returns the created ticket key. | `project` (string, **required**)<br>`summary` (string, **required**)<br>`description` (string, optional)<br>`steps` (string, optional) |
...
| `jira_xray_search_tickets` | Search for Jira tickets using JQL query and enrich Test/Precondition issues with X-ray test steps and preconditions. Returns list of tickets with X-ray data. | `searchQueryJQL` (string, **required**)<br>`fields` (array, optional) |
...
```javascript
// In JavaScript agent
const result = jira_add_fix_version("key", "fixVersion");
```
...
```javascript
// In JavaScript agent
const result = jira_assign_ticket_to("key", "accountId");
```
...
```javascript
// In JavaScript agent
const result = jira_assign_workflow_scheme("workflowSchemeId", "projectId");
```
...
```javascript
// In JavaScript agent
const result = jira_attach_file_to_ticket("ticketKey", "name");
```
...
```javascript
// In JavaScript agent
const result = jira_clear_field("key", "field");
```
...
```javascript
// In JavaScript agent
const result = jira_create_project_issue_type("projectKey", "name");
```
...
```javascript
// In JavaScript agent
const result = jira_create_ticket_basic("project", "issueType");
```
...
```javascript
// In JavaScript agent
const result = jira_create_ticket_with_parent("project", "issueType");
```
...
```javascript
// In JavaScript agent
const result = jira_get_comments("key", "ticket");
```
...
```javascript
// In JavaScript agent
const result = jira_get_ticket("key", "fields");
```
...
```javascript
// In JavaScript agent
const result = jira_link_issues("sourceKey", "anotherKey");
```
...
```javascript
// In JavaScript agent
const result = jira_move_to_status("key", "statusName");
```
...
```javascript
// In JavaScript agent
const result = jira_move_to_status_with_resolution("key", "statusName");
```
...
```javascript
// In JavaScript agent
const result = jira_remove_fix_version("key", "fixVersion");
```
...
```javascript
// In JavaScript agent
const result = jira_search_by_page("jql", "nextPageToken");
```
...
```javascript
// In JavaScript agent
const result = jira_search_with_pagination("jql", "startAt");
```
...
```javascript
// In JavaScript agent
const result = jira_set_fix_version("key", "fixVersion");
```
...
```javascript
// In JavaScript agent
const result = jira_set_priority("key", "priority");
```
...
```javascript
// In JavaScript agent
const result = jira_update_all_fields_with_name("key", "fieldName");
```
...
```javascript
// In JavaScript agent
const result = jira_update_description("key", "description");
```
...
```javascript
// In JavaScript agent
const result = jira_update_field("key", "field");
```
...
```javascript
// In JavaScript agent
const result = jira_update_field_as_adf("key", "field");
```
...
```javascript
// In JavaScript agent
const result = jira_update_ticket("key", "params");
```
...
```javascript
// In JavaScript agent
const result = jira_xray_add_preconditions_to_test("testIssueId", "preconditionIssueIds");
```
...
```javascript
// In JavaScript agent
const result = jira_xray_add_test_step("issueId", "action");
```
...
```javascript
// In JavaScript agent
const result = jira_xray_add_test_steps("issueId", "steps");
```
...
```javascript
// In JavaScript agent
const result = jira_xray_create_precondition("project", "summary");
```
...
```javascript
// In JavaScript agent
const result = jira_xray_search_tickets("searchQueryJQL", "fields");
```
